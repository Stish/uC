...
UINT i2cPutbyte(u08 b)
{
	...
	I2C_SCL_HI;					// clock back up
  	b = inp(SDAPIN) & (1<<SDA);	// get the ACK bit
...
void i2cSend(u08 _device, u08 _subAddr, u08 length, u08 *data)
{
	I2C_START;      			// do start transition
	u08 device = _device << 4 | _subAddr << 1;
...
void i2cReceive(u08 _device, u08 _subAddr, u08 length, u08 *data)
{
	int j = length;
	u08 *p = data;
	...
	/*I2C_START;					// do start transition
	i2cPutbyte( (device << 4 | subAddr) );			// send DEVICE address
	//i2cPutbyte(subAddr);   		// and the subaddress
	HDEL;
...
void i2cReceives(u08 _device, u08 _subAddr, u08 _lengths, u08 *_send_data, u08 length, u08 *_recv_data)
{
	int j = _lengths;
	u08 *p = _recv_data;
	u08 *s = _send_data;
	...
	i2cPutbyte( device );			// send DEVICE address
	while (j--) 
		i2cPutbyte(*s++);   		// and the subaddress
